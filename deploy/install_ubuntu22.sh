...
#   - A dedicated 'leadcrm-voip-lab' system user and /opt/leadcrm-voip-lab install directory
...
DB_NAME="leadcrm_voip_lab"
DB_USER="leadcrm_voip_lab"
...
id -u leadcrm-voip-lab &>/dev/null || useradd --system --create-home --shell /usr/sbin/nologin leadcrm-voip-lab
mkdir -p "$INSTALL_DIR"
...
if [[ -f /etc/nginx/sites-available/leadcrm-voip-lab ]]; then
  echo "Nginx site already exists at /etc/nginx/sites-available/leadcrm-voip-lab - leaving it untouched"
  echo "(so any server_name/TLS changes you made aren't overwritten)."
else
  cp "$SCRIPT_DIR/nginx_leadcrm_voip_lab.conf" /etc/nginx/sites-available/leadcrm-voip-lab
fi
...
echo
echo "=================================================================="
echo " Lead CRM VOIP Lab installed."
echo
...
echo " Backend service:     systemctl status leadcrm-voip-lab-backend"
echo " Nginx site:          /etc/nginx/sites-available/leadcrm-voip-lab"
echo
...
echo
echo " IMPORTANT:"
echo "  1. Edit /etc/nginx/sites-available/leadcrm-voip-lab and set server_name to"
