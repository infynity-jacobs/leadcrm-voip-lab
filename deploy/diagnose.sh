...
if systemctl is-active --quiet leadcrm-voip-lab-backend; then
  echo "OK  leadcrm-voip-lab-backend.service is active"
else
  echo "FAIL  leadcrm-voip-lab-backend.service is NOT active"
  echo "      (systemd may still be restart-looping it - see logs below)"
  PROBLEMS=1
fi
echo "--- last 15 log lines ---"
journalctl -u leadcrm-voip-lab-backend -n 15 --no-pager 2>&1 | sed 's/^/    /'

if journalctl -u leadcrm-voip-lab-backend -n 50 --no-pager 2>&1 | grep -q "pydantic_core.*ValidationError\|extra_forbidden"; then
  echo
...
if [[ "$PROBLEMS" -eq 0 ]]; then
  ...
  echo "   sudo systemctl restart leadcrm-voip-lab-backend"
  echo
  ...
  echo "   sudo journalctl -u leadcrm-voip-lab-backend -n 100 --no-pager"
else
