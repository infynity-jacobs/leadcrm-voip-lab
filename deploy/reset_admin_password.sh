#!/usr/bin/env bash
#
# Resets the password for an existing Lead CRM VOIP Lab user directly in the
...
# Run as root (needs to read /opt/leadcrm-voip-lab/backend/.env and use its venv).
#
set -euo pipefail
...
if [[ ! -f "$ENV_FILE" ]]; then
  echo "ERROR: $ENV_FILE not found. Is Lead CRM VOIP Lab installed at $INSTALL_DIR?" >&2
  exit 1
fi
if [[ ! -x "$VENV_PY" ]]; then
...
import sys
sys.path.insert(0, "/opt/leadcrm-voip-lab/backend")
