...
Views.settings = async function (root) {
  ...
  root.innerHTML = `
    ...
    <ul class="nav nav-tabs mb-3" role="tablist">
      ...
      <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#voip-tab">VOIP</button></li>
    ...
    <div class="tab-content">
      ...
      <div class="tab-pane fade" id="voip-tab"><div class="card"><div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center mb-2">
          <div><h6 class="mb-1">Yeastar Extensions</h6><p class="text-muted small mb-0">Read-only extension discovery from the S50. Map an extension to one CRM user; SIP credentials are not stored here.</p></div>
          <button type="button" class="btn btn-outline-primary btn-sm" id="voip-refresh-extensions"><i class="bi bi-arrow-clockwise"></i> Refresh</button>
        </div>
        <div id="voip-extension-result" class="small"></div>
        <div class="table-responsive">
          <table class="table table-sm align-middle mb-0" id="voip-extension-table">
            <thead class="table-light"><tr><th>Extension</th><th>Name</th><th>Type</th><th>Status</th><th>CRM User</th><th></th></tr></thead>
            <tbody><tr><td colspan="6" class="text-center text-muted py-3">Click Refresh to load extensions.</td></tr></tbody>
          </table>
        </div>
        <div class="alert alert-info small mt-3 mb-0"><strong>v2.9 scope:</strong> extension discovery and CRM-user mapping only. No calls are placed and no Yeastar extension settings are modified.</div>
      </div></div></div>
  ...
  const extensionResult = qs("#voip-extension-result");
  const extensionTbody = qs("#voip-extension-table tbody");
  const renderExtensionRows = (extensions, mappings, users) => {
    const mapByExt = Object.fromEntries((mappings || []).map(m => [String(m.extension_number), m]));
    const activeUsers = (users || []).filter(u => u.is_active);
    if (!extensions.length) {
      extensionTbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted py-3">No extensions returned by the Yeastar PBX.</td></tr>`;
      return;
    }
    extensionTbody.innerHTML = extensions.map(ext => {
      const m = mapByExt[String(ext.number)];
      const options = [`<option value="">Unassigned</option>`].concat(activeUsers.map(u => `<option value="${u.id}" ${m && Number(m.user_id)===Number(u.id)?"selected":""}>${escapeHtml(u.full_name)} (${escapeHtml(u.username)})</option>`)).join("");
      return `<tr data-extension="${escapeHtml(ext.number)}" data-mapping-id="${m ? m.id : ""}">
        <td class="fw-semibold text-nowrap">${escapeHtml(ext.number)}</td>
        <td>${escapeHtml(ext.name || "-")}</td>
        <td>${escapeHtml(ext.type || "-")}</td>
        <td><span class="badge ${ext.status === "Registered" ? "text-bg-success" : ext.status === "Idle" ? "text-bg-secondary" : "text-bg-warning"}">${escapeHtml(ext.status || "Unknown")}</span></td>
        <td><select class="form-select form-select-sm voip-extension-user">${options}</select></td>
        <td class="text-nowrap"><button type="button" class="btn btn-primary btn-sm voip-save-extension">Save</button>${m ? ` <button type="button" class="btn btn-outline-danger btn-sm voip-remove-extension" title="Unassign">×</button>` : ""}</td>
      </tr>`;
    }).join("");
  };

  const loadYeastarExtensions = async () => {
    extensionResult.innerHTML = '<span class="text-muted">Loading Yeastar extensions…</span>';
    try {
      const [data, users] = await Promise.all([apiFetch("/voip/extensions"), apiFetch("/users")]);
      renderExtensionRows(data.extensions || [], data.mappings || [], users || []);
      extensionResult.innerHTML = data.ok ? `<span class="text-success">${escapeHtml(data.message || "Extensions loaded.")}</span>` : `<span class="text-danger">${escapeHtml(data.message || "Extension discovery failed.")}</span>`;
    } catch (e) {
      extensionTbody.innerHTML = `<tr><td colspan="6" class="text-center text-danger py-3">${escapeHtml(e.detail || "Failed to load Yeastar extensions")}</td></tr>`;
      extensionResult.textContent = "";
    }
  };

  qs("#voip-refresh-extensions")?.addEventListener("click", loadYeastarExtensions);
  extensionTbody?.addEventListener("click", async (event) => {
    const row = event.target.closest("tr[data-extension]");
    if (!row) return;
    ...
    if (event.target.closest(".voip-remove-extension")) {
      const id = Number(row.dataset.mappingId || 0);
      if (!id || !confirm(`Unassign extension ${row.dataset.extension}?`)) return;
      try { await apiFetch(`/voip/extensions/mapping/${id}`, {method:"DELETE"}); showToast("Extension unassigned"); await loadYeastarExtensions(); }
      catch (e) { showToast(e.detail || "Failed to remove extension mapping", "danger"); }
    }
  });

  loadYeastarExtensions();
