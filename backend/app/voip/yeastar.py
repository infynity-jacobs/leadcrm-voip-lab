...
class YeastarAdapter(VoIPProviderAdapter):
    ...
    def query_extensions(self, config: Dict[str, Any]) -> VoIPAdapterResult:
        """Read extension number/name/status from the S-Series API without exposing PBX secrets."""
        ...
        login = self._request_json(f"{base}/login", {
            ...
            "version": api_version,
            "port": str(config.get("event_port") or "0"),
        }, verify_tls)
        ...
        try:
            response = self._request_json(f"{base}/extension/query?token={token}", {"number": "all"}, verify_tls)
            if str(response.get("status", "")).lower() != "success":
                return VoIPAdapterResult(False, self.provider_key, self._failure_message(response, "Yeastar extension query failed"), {"yeastar_errno": response.get("errno")})
            safe = []
            for ext in response.get("extinfos") or []:
                number = str(ext.get("number") or "").strip()
                if not number:
                    continue
                safe.append({
                    "number": number,
                    "name": str(ext.get("username") or ext.get("callerid") or number),
                    "status": str(ext.get("status") or "Unknown"),
                    "type": str(ext.get("type") or ""),
                    "callerid": str(ext.get("callerid") or ""),
                })
            safe.sort(key=lambda x: (x["number"].isdigit() is False, int(x["number"]) if x["number"].isdigit() else x["number"]))
            return VoIPAdapterResult(True, self.provider_key, f"{len(safe)} Yeastar extensions received.", {"extensions": safe})
        finally:
            try:
