# Lead CRM — VOIP Lab Build v2.8.0

> **This is a separate testing build. It is not intended to replace or upgrade the production Lead CRM.**

## Purpose
This build establishes a provider-neutral VOIP foundation so the CRM can later integrate with whichever PBX/SIP/VOIP platform is selected. **No Yeastar adapter is included.** No live calls are placed or received by this build.

## Isolated installation
The supplied installer uses these separate defaults:

- Application: `/opt/leadcrm-voip-lab`
...
Install this on a **separate Ubuntu 22.04 server**. Do not run it against the production CRM database.

## Current VOIP scope
Settings → VOIP provides provider-neutral configuration for:

- Enable/disable VOIP
- Generic SIP / WebRTC
- Asterisk / FreePBX (future integration option)
- SIP trunk / VOIP provider (future integration option)
- PBX API / WebSocket (future integration option)
- Other
- PBX/SIP host and port
- Browser WebSocket URL
- Domain / realm
- STUN / TURN
- Per-user or shared extension model

The selected provider type is only configuration metadata at this stage; selecting an option does not activate an adapter.

## Planned next steps
1. Evaluate the provider-neutral settings and browser requirements.
2. Decide which VOIP/PBX integration should be implemented first.
3. Add per-user extension/credential storage with encryption.
4. Add browser softphone and click-to-call.
5. Integrate call events with the existing Call Log.
6. Add incoming caller identification, outcomes and follow-ups.

## Installation
Run on the dedicated test server:

```bash
sudo bash deploy/install_ubuntu22.sh
```

Then edit `/etc/nginx/sites-available/leadcrm-voip-lab`, set the test hostname, and configure HTTPS before browser WebRTC testing.

See `VOIP_LAB_PLAN.md` for the intended scope.
